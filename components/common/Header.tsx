    ...
    return (
        <>
            <header className="bg-white relative">
                <div className="display-flex py-3.75 px-[5%] max-w-300
                    pc:py-3.75 pc:px-0 pc:my-0 pc:mx-auto">
                    <div>
                        <Link href="/">
                            <Image
                                className="w-30 h-auto pc:w-42.5"
                                src="/icons/logo_black.png" alt="에코원" width={300} height={95} />
                        </Link>
                    </div>

                    <nav className={`fixed top-0 w-full h-dvh bg-white
                            transition-all duration-300 z-40 py-3.75 px-[5%]
                            pc:static pc:inset-auto pc:w-auto pc:h-auto pc:p-0 pc:my-auto pc:mx-0
                            ${isOpen ? "right-0" : "-right-full"}`}>
                        <div className="mo mb-7.5 pc:my-auto mx-0">
                            <div className="flex justify-end" onClick={() => setIsOpen(false)}>
                                <Image
                                    className="w-7.5 h-auto cursor-pointer"
                                ...
                                return (
                                    <li key={key} className="pc:relative">
                                        ...
                                        {c.categories && isOpenSub === key && (
                                            <ul className="bg-[#f5f5f5] rounded-lg p-2.5
                                            pc:absolute pc:w-45 pc:left-0 pc:top-12.5 pc:bg-white pc:rounded-0 pc:border pc:border-[#ccc] pc:p-0">
                                                {c.categories.map((sub) => (
                                                    <li
                                                        className="py-2.5 text-[1rem] pc:py-2.5 pc:px-5"
                                                        key={sub.url}
                                                        onClick={() => { setIsOpenSub(null); setIsOpen(false); }}>
                                                        <Link
                    ...
                    <div className="mo cursor-pointer my-auto mx-0" onClick={() => setIsOpen(true)}>
                        <Image
                            className="w-7.5 h-auto my-auto mx-0
                            pc:w-7.5 pc:h-auto pc:my-auto pc:mx-0"
